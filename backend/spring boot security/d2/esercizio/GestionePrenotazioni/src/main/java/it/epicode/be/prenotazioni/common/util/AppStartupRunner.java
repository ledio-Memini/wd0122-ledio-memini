...
public class AppStartupRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		if (cittaRepository.count() == 0) {
			...
			User user = saveUser("m.rossi", "Mario Rossi", "m@nowhere.no","test",new HashSet(role));
	...
	private User saveUser(String username, String nome, String email , String plainPassword, Set<Role> roles) {
		User user = new User();
	...
	private List<Edificio> initEdificio(Citta initCitta) {
		...
		edificio.setSecurityCode("very secure code 1");
		...
		edificio.setSecurityCode("very secure code 2");
